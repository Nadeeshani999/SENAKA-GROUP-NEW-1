'use strict';
/* ═══════════════════════════════════════════════
   SENAKA GROUP DASHBOARD — app.js v6.1
...
const Charts = {
  ...
  /* ══════════════════════════════════════════
     BANK DOT PLOT — compact, capped height
     Uses dropdown filtering to reduce rows.
     Max height 400px. 28px per row minimum.
  ══════════════════════════════════════════ */
  bankDotPlot(id, rows) {
    this.destroy(id);
    const canvas = document.getElementById(id); if (!canvas) return;
    ...
    /* Compact height: 28px per dot, min 200px, max 400px */
        ...
        ticks: {
          ...
          callback: v => {
            if (Number.isInteger(v) && v >= 0 && v < n) {
              const r = sorted[v];
              /* Short label: abbreviated bank + last 4 digits */
    ...
    const bgs     = sorted.map(r => (r['Amount'] || 0) < 0 ? 'rgba(244,63,94,0.85)'  : 'rgba(0,200,255,0.85)');
    const borders = sorted.map(r => (r['Amount'] || 0) < 0 ? '#b91c3c'                : '#0081a8');
...
/* ═══════════════════════════════════════════════
   ...
   • Bank dropdown     (NEW — filters by bank name)
   • Dot plot is compact: max 400px height
═══════════════════════════════════════════════ */
async function initBankPage() {
  ...
  /* ── Bank name slicer (NEW) ── */
  const bankNames = [...new Set(allRows.map(r => r['Bank Name']).filter(Boolean))].sort();
  const bankSel   = document.getElementById('bank-bank-sel');
  if (bankSel) {
  ...
  function getFiltered() {
    ...
    if (bank) rows = rows.filter(r => r['Bank Name'] === bank);
    return rows;
  }
  ...
  function renderView(rows) {
    ...
    /* ── Compact dot plot — filtered rows keep it small ── */
    ...
    /* ── Stacked bar — credit vs debit per bank group ── */
    const bankGroups = {};
    rows.forEach(r => {
      const k = groupKey(r['Bank Name'] || 'Other');
  ...
  bankSel?.addEventListener('change', () => { Sound.click(); renderView(getFiltered()); });
