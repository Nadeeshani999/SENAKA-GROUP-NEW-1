...
const Charts = {
  ...
  /* ── NEW: dot plot for bank balances ── */
  bankDotPlot(id, rows) {
    this.destroy(id);
    const canvas = document.getElementById(id); if (!canvas) return;
    ...
    /* dynamically resize parent box so every dot has room */
    const box = canvas.parentElement;
    if (box) box.style.height = Math.max(n * 34 + 80, 280) + 'px';

    const cfg = this._base();
    cfg.animation = { duration: 800, easing: 'easeOutQuart' };
    cfg.layout   = { padding: { left: 6, right: 28, top: 8, bottom: 8 } };
    ...
    cfg.scales = {
      x: {
        ...axisStyle,
        type: 'linear',
        title: { display: true, text: 'Balance (LKR)', color: C.muted, font: { family:"'Inter',system-ui,sans-serif", size: 9 } },
        ticks: {
          ...axisStyle.ticks,
          maxTicksLimit: 7,
        ...
        ticks: {
          ...
          callback: v => {
            if (Number.isInteger(v) && v >= 0 && v < n) {
              const r = sorted[v];
              const name = (r['Bank Name'] || '').replace('Peoples Bank ','PB ').replace('Commercial - ','Comm. ').replace('Sampath Bank-','Sampath ').replace('Seylan – ','Seylan ').replace('Seylan - ','Seylan ');
              return name + ' · ' + (r['Account Number'] || '');
            }
            return '';
          }
        }
...
async function initBankPage() {
  ...
  function renderView(rows) {
    ...
    /* ── REPLACED CHART: dot plot instead of log-scale bar ── */
