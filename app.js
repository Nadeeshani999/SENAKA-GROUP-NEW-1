...
/* ═══════════════════════════════════════════════
   ...
   Fix: Bank Bar chart Y-axis now shows Account Number
        Dot correctly shows Balance on X-axis
═══════════════════════════════════════════════ */
...
const Charts = {
  ...
  /* ═══════════════════════════════════════════
     BANK DOT PLOT — FIXED v6.3
     Y-axis: "Bank Name  ···AccNo" (last 6 digits)
     X-axis: Balance (LKR) — dot positioned by amount
     Tooltip: full bank name + full account number + balance
  ═══════════════════════════════════════════ */
  bankDotPlot(id, rows) {
    this.destroy(id);
    ...
    /* Sort ascending by balance so lowest debt is at bottom */
    const sorted = [...rows].sort((a, b) => (a['Amount'] || 0) - (b['Amount'] || 0));
    const n = sorted.length;

    /* Dynamic height — give each row ~30px, min 220, max 520 */
    const box = canvas.parentElement;
    if (box) box.style.height = Math.min(Math.max(n * 30 + 60, 220), 520) + 'px';

    const cfg = this._base();
    cfg.animation = { duration: 900, easing: 'easeOutQuart' };
    cfg.layout    = { padding: { left: 8, right: 32, top: 10, bottom: 10 } };
    ...
    cfg.scales = {
      ...
      x: {
        ...
        ticks: {
          ...axisStyle.ticks,
          maxTicksLimit: 7,
          callback: v => fmtLKR(v, true)   /* compact: 1.50Cr, 2.30L … */
        }
      },
      /* ── Y axis: one row per account ── */
      y: {
        ...axisStyle,
        type: 'linear',
        min: -0.8,
        ...
        ticks: {
          color: C.tick,
          font: { family: "'Inter',system-ui,sans-serif", size: 8.5 },
          stepSize: 1,
          /* Show "BankName  ···123456" — last 6 digits of account */
          callback: v => {
            if (!Number.isInteger(v) || v < 0 || v >= n) return '';
            const r   = sorted[v];
            const bank = (r['Bank Name'] || '')
              .replace('Peoples Bank ', 'PB ')
              .replace('People\'s Bank ', 'PB ')
              .replace('Commercial Bank - ', 'Comm. ')
              .replace('Commercial - ', 'Comm. ')
              .replace('Sampath Bank-', 'Sampath ')
              .replace('Sampath Bank - ', 'Sampath ')
              .replace('Seylan Bank - ', 'Seylan ')
              .replace('Seylan – ', 'Seylan ')
              .replace('Seylan - ', 'Seylan ');
            /* Show last 6 chars of account number for identification */
            const acc  = String(r['Account Number'] || '').replace(/\s/g, '');
            const tail = acc.length > 6 ? '···' + acc.slice(-6) : acc;
    ...
    cfg.plugins.tooltip.callbacks = {
      title: ctx => {
        const r = sorted[ctx[0].parsed.y];
        return (r['Bank Name'] || '—');
      },
      label: ctx => {
        const r = sorted[ctx[0].parsed.y];
        const lines = [];
        lines.push('Acc: ' + (r['Account Number'] || '—'));
        if (r['Account Type']) lines.push('Type: ' + r['Account Type']);
        const amt = ctx.parsed.x;
        lines.push('Balance: Rs. ' + fmtLKR(amt));
        if (r['Last Updated']) lines.push('Updated: ' + fmtDate(r['Last Updated']));
        return lines;
      }
    };

    /* Colour: cyan for positive/zero, red for negative/overdrawn */
    const bgs     = sorted.map(r => (r['Amount'] || 0) < 0
      ? 'rgba(244,63,94,0.90)'
      : 'rgba(0,200,255,0.88)');
    const borders = sorted.map(r => (r['Amount'] || 0) < 0
      ? '#f43f5e'
      : '#00c8ff');
    ...
    State.charts[id] = new Chart(canvas, {
      ...
      data: {
        datasets: [{
          /* Each point: x = balance amount, y = row index in sorted array */
...
async function initBankPage() {
  ...
  function renderView(rows) {
    ...
    /* ── Dot plot: Y = account label, X = balance, dot = circle ── */
    Charts.bankDotPlot('bank-bar', rows);

    /* ── Donut: positive balances by bank group ── */
    const donutGroups = {};
